...
// Review 3, finding 2: a record evicted mid-pull must not let the pull
// return success with its layers unreferenced. The pull rewrites the record
// after the unpack loop, so even if eviction (or anything else) removed the
// pre-written record while the pull was in flight, success implies the
// record exists and the layers are reachable.
...
// DESIGN-V2 §8 obligation: the per-layer progress touch keeps a slow pull's
// record fresh mid-flight — freshness must come from layer completions, not
// only from the final rewrite.
...
// DESIGN-V2 §8 obligation: if a layer dir is yanked mid-pull despite
// everything (the residual hole), the pull must fail cleanly — a retryable
// error — rather than returning LayerDirs that point at nothing.
